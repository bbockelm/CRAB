...
DelegationRPMs.sh 
A script to install/uninstall and configure the RPMs needed for User Proxy Delegation ("root" permissions are required)

* Installation Syntax:
DelegationRPMs.sh install -path </your/dir> [-user <crabuser>] [-homeuser <crabuser Home dir>]

-path </your/dir>            : location of where the RPMs will be installed  
...
List="$CA_RPM_list $LCG_RPM_list $VDT_RPM_list $API2_RPM_list $API_RPM_list $PROXY_RPM_list $GRIDSITEDEV_RPM_list"
RPMList="";
