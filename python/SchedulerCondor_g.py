...
class SchedulerCondor_g(Scheduler):
    ...
    def wsSetupEnvironment(self):
        ...
        txt += '    middleware=OSG \n'
        txt += '    echo "SyncCE=`echo $OSG_JOB_CONTACT | awk -F \'/\' \'{print $1}\'`" | tee -a $RUNTIME_AREA/$repo \n'
    ...
    def wsCopyOutput(self):
        ...
        if int(self.copy_data) == 1:
           ...
           txt += '            echo "srmcp -retry_num 5 -retry_timeout 480000 -x509_user_trusted_certificates $X509_CERT_DIR file:////`pwd`/$out_file srm://${SE}:8443${SE_PATH}$out_file"\n'
           txt += '            exitstring=`srmcp -retry_num 5 -retry_timeout 480000 -x509_user_trusted_certificates $X509_CERT_DIR file:////\`pwd\`/$out_file srm://${SE}:8443${SE_PATH}$out_file 2>&1`\n'
           txt += '            copy_exit_status=$?\n'
