...
class SchedulerCondor_g(Scheduler):
    ...
    def wsCopyOutput(self):
        ...
        if int(self.copy_data) == 1:
           ...
           txt += '        echo "lcg-cp --vo $VO --verbose -t 1200 file://`pwd`/$out_file gsiftp://${SE}${SE_PATH}$out_file"\n'
           txt += '        exitstring=`lcg-cp --vo $VO --verbose -t 1200 file://\`pwd\`/$out_file gsiftp://${SE}${SE_PATH}$out_file 2>&1`\n'
