...
def getSoftwareAndArch(host_list, software, arch, bdii='exp-bdii.cern.ch'):
	...
	pout = runldapquery(query, 'GlueHostApplicationSoftwareRunTimeEnvironment GlueSubClusterUniqueID', bdii)
	output = concatoutput(pout)

	r = re.compile('^GlueHostApplicationSoftwareRunTimeEnvironment: (.*)')
	s = re.compile('^GlueSubClusterUniqueID: (.*)')
	stanzas = output.split(LF + LF)
	for stanza in stanzas:
...
if __name__ == '__main__':
	#print jm_from_se_bdii(sys.argv[1])
        print getJobManagerList(["srm.ihepa.ufl.edu","cmssrm.fnal.gov"],"CMSSW_2_0_0","slc4_ia32_gcc345")
