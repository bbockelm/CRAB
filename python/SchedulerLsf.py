...
class SchedulerLsf(Scheduler) :
    ...
    def configure(self, cfg_params):
        ...
        self.lsf_queue = None
        if (cfg_params.has_key('LSF.queue')): self.lsf_queue = cfg_params['LSF.queue']

        self.lsf_res = None
        if (cfg_params.has_key('LSF.resource')): self.lsf_res = cfg_params['LSF.resource']
    ...
    def sched_parameter(self):
        ...
        for i in range(len(first)): # Add loop DS
            ...
            if (self.lsf_queue):
                param_file.write('queue = '+self.lsf_queue +';\n')
                if (self.lsf_res): param_file.write('requirement = '+self.lsf_res +';\n')
            pass
    ...
    def wsSetupEnvironment(self):
        ...
        txt += 'MonitorJobID=`echo ${NJob}_$LSB_BATCH_JID`\n'
        txt += 'SyncGridJobId=`echo $LSB_BATCH_JID`\n'
